...
func (d *DB) UpdateMemoExport(ctx context.Context, update *store.UpdateMemoExport) error {
	...
	if update.ExportTs != nil {
		set, args = append(set, "export_ts = "+placeholder(len(args)+1)), append(args, *update.ExportTs)
	}
	...
	_, err := d.db.ExecContext(ctx, "UPDATE memo_export SET "+strings.Join(set, ", ")+" WHERE memo_id = "+placeholder(len(args)), args...)
	return err
}
